...
for path in paths:
    ...
    signals= ["signalResponseTimeCheckOutAndWrap","signalResponseTimeCheckOut","signalWaitingTimeCheckOutAndWrap","signalWaitingTimeCheckOut","Cumulative_response_time"]
    for y in range(len(signals)):
        for k in range(len(files)):

            new_filename = rchop(files[k], '.csv')
            f = open(path + "/" + new_filename + "_"+signals[y]+".txt", "a")
            f.close()
